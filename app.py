    ...
    with nav_col2:
        ...
        with b4:
            if st.button("STUDIO ✨", key="nav_studio", use_container_width=True):
                ...
                set_view("stylesync")
        with b5:
            if st.button("BEAUTY", key="nav_beauty_tab", use_container_width=True):
                st.session_state["active_modal"] = "beauty_collection"
                st.toast("💄 Showing Beauty & Grooming Picks")
                st.rerun()

    with nav_col3:
        search_query = st.text_input("🔍 Search products, brands...", placeholder="Type 'blazer', 'linen', 'zara'...", label_visibility="collapsed", key="search_bar")
        if search_query:
...
def render_drawers_and_modals() -> None:
    ...
    if active_m:
        if active_m == "women_collection":
            ...
            with col_w1:
                if st.button("🛍️ View Women's Linen Co-ord (₹2,999)", key="w_item1", use_container_width=True):
                    st.toast("Added Women's Co-ord to Bag!")
                    st.session_state["bag_count"] += 1
            with col_w2:
                if st.button("✖️ Close View", key="close_w_btn", use_container_width=True):
                    st.session_state["active_modal"] = None
                    st.rerun()
            ...
            with col_b1:
                if st.button("🧴 Add Woody Citrus Eau De Parfum (₹1,899)", key="b_item1", use_container_width=True):
                    st.toast("Added Parfum to Bag!")
                    st.session_state["bag_count"] += 1
            with col_b2:
                if st.button("✖️ Close View", key="close_b_btn", use_container_width=True):
                    st.session_state["active_modal"] = None
                    st.rerun()
            ...
            with s_c1:
                if st.button(f"👉 View '{TARGET_ITEM['name']}' PDP", key="search_match_btn", type="primary", use_container_width=True):
                    st.session_state["active_modal"] = None
                    set_view("pdp")
            with s_c2:
                if st.button("✖️ Clear Search", key="clear_s_btn", use_container_width=True):
                    st.session_state["active_modal"] = None
                    st.rerun()
            ...
            if st.button("✖️ Close Size Chart", key="close_sc_btn", use_container_width=True):
                st.session_state["active_modal"] = None
                st.rerun()
...
def render_homepage_view() -> None:
    ...
    with t_col2:
        if st.button("📋 Copy Code 'MYNTRASAVE'", key="copy_coupon_btn", use_container_width=True):
    ...
    with h_col2:
        ...
        with c_btn1:
            if st.button("🔥 EXPLORE NOW", key="hero_explore_btn", type="primary", use_container_width=True):
                ...
                set_view("pdp")
        with c_btn2:
            if st.button("✨ Style with Closet", key="hero_stylesync_btn", use_container_width=True):
                st.toast("Opening StyleSync Wardrobe Matcher...")
                set_view("stylesync")
        with c_btn3:
            if st.button("❤️ Save to Wishlist", key="hero_wl_btn", use_container_width=True):
                st.session_state["wishlist_count"] += 1
    ...
    with cat1:
        ...
        if st.button("Explore Men's →", key="cat_men_btn", use_container_width=True):
    ...
    with cat2:
        ...
        if st.button("Explore Women's →", key="cat_women_btn", use_container_width=True):
            st.session_state["active_modal"] = "women_collection"
            st.rerun()
    ...
    with cat3:
        ...
        if st.button("Explore Beauty →", key="cat_beauty_btn", use_container_width=True):
            st.session_state["active_modal"] = "beauty_collection"
            st.rerun()
    ...
    with cat4:
        ...
        if st.button("Explore Home →", key="cat_home_btn", use_container_width=True):
...
def render_pdp_view() -> None:
    ...
    with col_img:
        ...
        with t1:
            ...
            if st.button("📸 Front View", key="thumb_front", use_container_width=True):
                st.session_state["pdp_active_img"] = TARGET_ITEM["image_url"]
                st.rerun()
        with t2:
            st.image(IMAGE_OLIVE_SHIRT, use_container_width=True)
            if st.button("🌿 Olive Pair", key="thumb_olive", use_container_width=True):
                st.session_state["pdp_active_img"] = IMAGE_OLIVE_SHIRT
                st.rerun()
        with t3:
            st.image(IMAGE_BLACK_TROUSERS, use_container_width=True)
            if st.button("👖 Trouser Pair", key="thumb_pant", use_container_width=True):
    ...
    with col_details:
        ...
        with sz_chart:
            if st.button("📏 Size Chart", key="sc_open_btn", use_container_width=True):
        ...
        with s1:
            if st.button("38", key="sz_38", type="primary" if curr_size == "38" else "secondary", use_container_width=True):
                ...
                st.rerun()
        with s2:
            if st.button("40 ⭐", key="sz_40", type="primary" if curr_size == "40" else "secondary", use_container_width=True):
                st.session_state["selected_size"] = "40"
        ...
        with pin_col1:
            pin_code = st.text_input("Delivery Pincode", placeholder="Enter 6-digit Pincode (e.g. 110001)", label_visibility="collapsed", key="pin_input")
        with pin_col2:
            if st.button("Check ⚡", key="pin_btn", use_container_width=True):
                if pin_code:
        ...
        if st.button("✨ View 3 Wardrobe Outfits & WhatsApp Poll →", key="pdp_to_stylesync_btn", type="primary", use_container_width=True):
